<template>
  <div v-tilt="{ 'glare': true, 'max-glare': 1.2, max: 5, scale: 1.03 }" class="card" :style="{'background-image': `url('${game.cover}')`}">
    <div class="more_info" :class="visibleItem ? 'show' : ''">
      ...
      <div class="content">
        ...
        <div class="game-categories">
          <el-tag size="mini" v-for="(c, index) in game.categories" :key="index" :type="$options.filters.typeColor()">{{ c }}</el-tag>
        </div>
        <label for="show-menu" class="menu-button">
          <el-dropdown size="small" type="primary">
            <i class="el-icon-more"></i>
            <el-dropdown-menu slot="dropdown" @mouseover.native="visibleItem = true" @mouseleave.native="visibleItem = false">
             <el-dropdown-item icon="el-icon-check" @click.native="$emit('detail-game', game.id)">Xem chi tiết</el-dropdown-item>
    ...
    <div class="card__content">
      ...
      <div class="card__content--description">
        ...
        <p class="text-reveal">
          <span v-html="$options.filters.stringToPattern(game.summany, 12, '<span>', '</span>', 3)">
          </span>
          <span v-html="$options.filters.stringToPattern(game.summany, 12, '<span><span>', '</span></span>', 3)">
          </span>
        </p>
      </div>
    </div>
...
<script>
  export default {
    name: 'GameItem',
    props: {
      game: { type: Object, function () { return {} } }
    },
    filters: {
      typeColor() {
      ...
      stringToPattern(string, len = 12, patternStart = '', patternEnd = '', get = 3) {
        let curr = len;
        let prev = 0;
        let words = [];
        let output = '';
        ...
        for (var i = 0; i < get; i++) {
          output += patternStart + (words[i] != undefined ? words[i].trim() : '') + (i + 1 == get && words.length > get ? '...': '') + patternEnd;
        }
        return output;
      }
    },
...
<style lang="scss" scoped>
  ...
  .card {
    ...
    &:before {
      ...
      width: 100%;
      position: absolute;
      background-image: linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(145, 184, 184, 0.4) 100%);
    }
    &:hover {
      &:hover {
        .more_info {
    ...
    &:hover {
      ...
      // paragraph reveal
      .text-reveal {
        /deep/> span:nth-child(2) > span,
        /deep/> span:nth-child(2) > span > span {
